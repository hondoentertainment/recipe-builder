...
def process_images(image_paths: list[Path], output_name: str) -> Path:
    print(f"\n=== Extract recipes from {len(image_paths)} image(s) ===")
    recipes = extract_recipes_from_images(image_paths)
    ...
    export_recipes_to_word(recipes, output_path, config.IMAGES_DIR)
